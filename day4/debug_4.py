# -*- coding: utf-8 -*-
"""
Created on Wed Dec 15 21:52:38 2021
...
def check_rows(board):
    for row in board.contents:
        if set(row).issubset(picked_nums):
            print(f'Winner: {board.id} (ROW) -> {get_score(board)},{picked_nums[-1]}')
            return True
    return False
...
def check_columns(board):
    for i in range(5):
        ...
        if set(column).issubset(picked_nums):
            print(f'Winner: {board.id} (COLUMN) -> {get_score(board)},{picked_nums[-1]}')
            return True
    
    return False

def draw_nums(a):
    board_array = [b for b in a]
    last_winner = None
    for num in draw:
        ...
        for board in board_array:            
            ...
            if row_status == True or col_status == True:     
                ...
                tmp_array.append(board)
                
        board_array = [b for b in tmp_array]
        if len(board_array) == 0:
            return last_winner

winner = draw_nums(board_array)
